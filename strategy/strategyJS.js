...
this.shadowRoot.addEventListener("DOMContentLoaded", () => {
  const chwytak = this.shadowRoot.querySelector(".propozycje");

  for (let n = 0; n < arrMiniaturek.length; n++) {
    const miniturka = this.shadowRoot.createElement("img");
  ...
  const chwytakMiniaturki = this.shadowRoot.querySelectorAll(".miniaturka");

  for (let n of chwytakMiniaturki) {
    n.addEventListener("click", () => {
      const lapacz = n.getAttribute("data-value");
      const inframe = this.shadowRoot.querySelector(".inframe");
  ...
  const lol = this.shadowRoot.querySelectorAll(".button-scroll");
  let pozycja = 0;
  ...
  for (let n of lol) {
    n.addEventListener("click", (event) => {
      ...
      console.log(lol);
      const element = this.shadowRoot.querySelector(".propozycje");
